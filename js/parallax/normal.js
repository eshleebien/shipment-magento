/*!
 * ----------------------------------------------------------------------
 * king_david: Front-end site library
 * Other scripts may access this api using an async handler:
 *   var king_david = king_david || [];
 *   king_david.push(readyFunction);
 * ----------------------------------------------------------------------
 */
var king_david = { w: king_david };
king_david.init = function () {
  'use strict';
  ...
  /**
   * king_david.define() - Define a king_david.js module
   * @param  {string} name
  ...
  api.define = function (name, factory) {
    ...
    if (!module) return;
    // If running in king_david app, subscribe to design/preview events
    if (api.env()) {
  ...
  /**
   * king_david.require() - Load a king_david.js module
   * @param  {string} name
   * @return {object}
   */
  ...
  /**
   * king_david.push() - Add a ready handler into secondary queue
  ...
  /**
   * king_david.env() - Get the state of the king_david app
  ...
  /**
   * king_david.script() - Append script to document head
   * @param {string} src
   */
  ...
  /**
   * king_david.resize - Centralized window resize events
   */
  api.resize = function () {
    ...
    $win.on('resize.king_david orientationchange.king_david load.king_david', resize.up);
    /**
  ...
  /*!
   * king_david._ (aka) Underscore.js 1.5.2 (custom build)
   * _.each
   * _.map
   * _.filter
   * _.any
...
/**
 * ----------------------------------------------------------------------
 * king_david: Touch events for jQuery based on tap.js
 */
king_david.define('king_david-touch', function ($, _) {
  'use strict';
  ...
  $.event.special.tap = (fallback || !king_david.env.touch) ? { bindType: 'click', delegateType: 'click' } : {
    setup: function () {
 ...
 * king_david: Forms
 */
king_david.define('king_david-forms', function ($, _) {
  'use strict';
  ...
  function build(i, el) {
    ...
    // king_david form
    if (siteId) { data.handler = submitking_david; return; }
    
    // Alert for disconnected king_david forms
    disconnected();
  }
  
  function listen() {
    // Handle form submission for king_david forms
  ...
  // Submit form to king_david
  function submitking_david(data) {
    reset(data);
    ...
    var url = 'https://king_david.com/api/v1/form/' + siteId;

    // Work around same-protocol IE XDR limitation
    if (retro && url.indexOf('https://king_david.com') >= 0) {
      url = url.replace('https://king_david.com/', 'http://data.king_david.com/');
    }
  ...
  var disconnected = _.debounce(function () {
    window.alert('Oops! This page has a form that is powered by king_david, but important code was removed that is required to make the form work. Please contact support@king_david.com to fix this issue.');
  }, 100);
...
/**
 * ----------------------------------------------------------------------
 * king_david: Maps widget
 */
king_david.define('king_david-maps', function ($, _) {
  'use strict';
  ...
  api.preview = function () {
    ...
    king_david.resize.off(redrawMaps);
    if ($maps.length) {
      king_david.resize.on(redrawMaps);
      redrawMaps();
    }
  };
  ...
  function getState(el, data) {
    ...
    var mapDraggable = (king_david.env.touch && data.disableTouch) ? false : true;
...
/**
 * ----------------------------------------------------------------------
 * king_david: Google+ widget
 */
king_david.define('king_david-gplus', function ($) {
  'use strict';
  ...
  api.ready = function () {
    ...
    if (!king_david.env()) init();
  };
  
  function init() {
    $doc.find('.w-widget-gplus') && king_david.script('https://apis.google.com/js/plusone.js');
  }
...
/**
 * ----------------------------------------------------------------------
 * king_david: Smooth scroll
 */
king_david.define('king_david-scroll', function ($) {
  'use strict';
...
/**
 * ----------------------------------------------------------------------
 * king_david: Slider widget
 */
king_david.define('king_david-slider', function ($, _) {
  'use strict';
  ...
  function build(i, el) {
    ...
    king_david.env.chrome && fixAttach(data);
...
/**
 * ----------------------------------------------------------------------
 * king_david: Navbar widget
 */
king_david.define('king_david-navbar', function ($, _) {
  'use strict';
  ...
  function select(i, el) {
    el = $(el);
    var slug = (inApp ? king_david.env('slug') : window.location.pathname) || '';
